...
fn program_module_deep_clone_test() {
    ...
    if let ProgramModuleOption::ProgramModule(Some(module)) = &module.options[2] {
        if let ProgramModuleOption::ProgramModule(Some(cloned)) = &cloned.options[2] {
            assert_ne!(module.id, cloned.id);
            if let ProgramModuleChildItems::Block(module) = &module.child {
                if let ProgramModuleChildItems::Block(cloned) = &cloned.child {
                    assert!(module.isomorphisms(cloned));
                    assert_ne!(module.id, cloned.id);
                    let module = &module.children;
                    let cloned = &cloned.children;
    ...
    if let ProgramModuleChildItems::MultiBlock(module) = &module.child {
        if let ProgramModuleChildItems::MultiBlock(cloned) = &cloned.child {
            assert_eq!(module.len(), cloned.len());
            for (module, cloned) in module.iter().zip(cloned) {
                assert!(module.isomorphisms(cloned));
                assert_ne!(module.id, cloned.id);
...
fn program_module_add_test() {
    ...
    let add = new_module();
    expect.options[3] = ProgramModuleOption::ProgramModule(Some(add.clone()));
    ...
    if let ProgramModuleOption::ProgramModule(Some(module)) = &mut expect.options[2] {
        if let ProgramModuleChildItems::Block(list) = &mut module.child {
            list.children.insert(3, add.clone());
        } else { unreachable!(); }
    } else { unreachable!(); }
    ...
    if let ProgramModuleChildItems::MultiBlock(lists) = &mut expect.child {
        lists[3].children.insert(3, add.clone());
    } else { unreachable!(); }
    assert_eq!(module.add(id, 3, add), Ok(()));
    assert_eq!(module, expect);
}
